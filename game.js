...
function speak(speech, enabled) {
  ...
  unlockVoice();
  void (async () => {
    try { await playUrl(await synth(speech), my); }
    catch (err) { console.debug("Zundamon unavailable:", err); }
  })();
}
...
let difficulty = "easy";
let voiceOn = localStorage.getItem("fanVoiceEnabled") !== "0";
let phase = "title";
let hold = false;
let chatSeq = 0;
let eventCooldown = 5;
...
function startGame() {
  unlockVoice(); resetSim(); phase = "playing";
  $("chatList").innerHTML = ""; pushChat(); show("play");
  setLine("idle", pick(LINES.idle), true); paintPlay();
}
  ...
  while (acc >= STEP) {
    ...
    if (sim.event) {
      ...
      if (eventCooldown <= 0) {
        ...
        pushChat();
        setLine("event", { text: ev.line, speech: ev.speech }, true);
      }
    }
    if (sim.nextChat <= 0) {
      pushChat();
    ...
    if (now - lastLineAt > 3200) {
      if (holdPct > 0.7 && canHold) setLine("reverseWarn", pick(LINES.reverseWarn));
      else if (sim.urge > 80) setLine("warn", pick(LINES.warn));
      else if (canHold) setLine("hold", pick(LINES.hold));
      else if (!sim.event) setLine("idle", pick(LINES.idle));
    }
...
$("voiceToggle").onclick = () => {
  voiceOn = !voiceOn;
  localStorage.setItem("fanVoiceEnabled", voiceOn ? "1" : "0");
  syncVoiceUI();
  if (!voiceOn) stopVoice();
  else { unlockVoice(); speak("すみ！", true); }
};
$("startBtn").onclick = startGame;
$("retryBtn").onclick = () => { show("title"); phase = "title"; $("wrap").style.transform = ""; };
...
syncDiffUI(); syncVoiceUI(); renderBest(); show("title");
requestAnimationFrame(tick);
